...
응답:
{
  ...
  "plan": [
    {
      "agent": "internal_rag",
      "query": "인덱싱된 문서에서 에이전틱 AI에 대해 설명해줘. 출처 파일명과 storage_ref도 알려줘."
    }
  ],
  "direct_answer": ""
}
...
예:
사용자: 2026-06-27 오후에 비어있는 시간 알려줘
응답:
{
  "intent": "CALENDAR_OPERATION",
  "plan": [
    {
      "agent": "calendar_agent",
      "query": "2026-06-27 오후에 비어있는 시간을 알려줘."
    }
  ],
  "direct_answer": ""
}
            ...
            if result:
                if hasattr(result, "artifacts") and result.artifacts:
                    ...
                    for artifact in result.artifacts:
                        ...
                        logger.info(
                            f"[ORCHESTRATOR] [REMOTE] artifact.name: {artifact.name}, "
                            f"parts: {len(artifact.parts)}"
                        )
